...
export function SoilDashboard({
  ...
  const radarData = useMemo(
    () => health.factors.map((f) => ({ factor: f.label.replace(/ \(.*\)/, ""), score: Math.round(f.score) })),
    [health.factors],
  );
    ...
    () =>
      [...history]
        ...
        .map((r) => {
          ...
          return {
            date: new Date(r.context.testedOn).toLocaleDateString("en-IN", { day: "2-digit", month: "short" }),
            score,
  ...
  const daysSince = Math.round((Date.now() - new Date(record.context.testedOn).getTime()) / 86_400_000);
    ...
    <div className="flex flex-col gap-5">
      {alerts.length > 0 && (
        <div className="grid gap-2">
          {alerts.map((a) => (
            <GlassCard key={a.id} className="p-3 flex items-start gap-2.5 border-warn/30 bg-warn/5">
        ...
        <div className="flex flex-col items-center gap-4 sm:flex-row sm:items-center sm:gap-6">
          ...
          <div className="flex-1 w-full grid gap-2.5">
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              <Pill tone={health.confidence >= 80 ? "good" : health.confidence >= 60 ? "warn" : "bad"}>
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">{health.confidenceBasis}</p>
            <div className="grid grid-cols-2 gap-2 text-xs">
              <Meta label="Last analysis" value={`${new Date(record.context.testedOn).toLocaleDateString("en-IN")} · ${daysSince}d ago`} />
              <Meta label="Field" value={`${record.context.fieldName || "Field"} · ${record.context.fieldSizeAcres} acre`} />
      ...
      <div className="grid grid-cols-2 gap-3">
        <StatTile label="Recommended crop" value={top ? `${top.crop} (${top.suitability}%)` : "—"} hint={top?.yieldPotential} tone="good" />
        <StatTile
          label="Recommended fertiliser"
          value={fertilizer.chemical[0]?.name ?? fertilizer.organic[0]?.name ?? "No corrective dose"}
          hint={fertilizer.chemical[0]?.quantity ?? fertilizer.organic[0]?.quantity}
        />
        <StatTile label="Sustainability" value={`${health.sustainability} / 100`} tone={toneForScore(health.sustainability)} hint="Carbon, salinity, structure, balance, water" />
        <StatTile
          label="Improvement progress"
          value={delta === null ? "Baseline test" : `${delta >= 0 ? "+" : ""}${delta} points`}
          tone={delta === null ? "neutral" : delta >= 0 ? "good" : "bad"}
          hint={previous ? `Since ${new Date(previous.context.testedOn).toLocaleDateString("en-IN")}` : "Record another test to track progress"}
        />
      </div>
      ...
      <GlassCard className="p-4 flex items-start gap-3">
        ...
        <div>
          <div className="text-[10px] font-bold uppercase tracking-[0.14em] text-muted-foreground">Irrigation recommendation</div>
      ...
      <GlassCard className="p-4 grid gap-4">
        <SectionTitle title="Nutrient profile" sub="Bars show the measured value against the agronomic optimum band." />
        <div className="grid gap-3.5 sm:grid-cols-2">
          <NutrientBar label="pH" value={m.ph} unit="" min={3} max={10} optimal={IDEAL.ph} digits={1} />
          <NutrientBar label="Nitrogen (N)" value={m.nitrogen} unit="kg/ha" min={0} max={700} optimal={IDEAL.nitrogen} />
          <NutrientBar label="Phosphorus (P)" value={m.phosphorus} unit="kg/ha" min={0} max={60} optimal={IDEAL.phosphorus} />
          <NutrientBar label="Potassium (K)" value={m.potassium} unit="kg/ha" min={0} max={500} optimal={IDEAL.potassium} />
          <NutrientBar label="Organic carbon" value={m.organicCarbon} unit="%" min={0} max={2.5} optimal={IDEAL.organicCarbon} digits={2} />
          <NutrientBar label="Moisture" value={m.moisture} unit="%" min={0} max={55} optimal={IDEAL.moisture} />
          <NutrientBar label="Electrical conductivity" value={m.ec} unit="dS/m" min={0} max={6} optimal={IDEAL.ec} digits={2} />
          <NutrientBar label="Water holding capacity" value={m.waterHoldingCapacity} unit="%" min={5} max={75} optimal={IDEAL.waterHoldingCapacity} />
        </div>
        <div className="flex items-center gap-2 text-xs text-muted-foreground border-t border-border pt-3">
          <FlaskConical className="size-3.5" />
          Soil temperature {m.temperature.toFixed(1)} °C — optimal {IDEAL.temperature[0]}–{IDEAL.temperature[1]} °C for microbial mineralisation.
        </div>
      </GlassCard>

      {/* Radar */}
      <GlassCard className="p-4">
        <SectionTitle title="Health factor balance" sub="Each axis is a weighted contributor to the overall score." />
        <div className="h-64 mt-2 -mx-2">
          <ResponsiveContainer width="100%" height="100%">
            <RadarChart data={radarData} outerRadius="72%">
              <PolarGrid stroke="var(--border)" />
              <PolarAngleAxis dataKey="factor" tick={{ fontSize: 10, fill: "var(--muted-foreground)" }} />
              <Radar dataKey="score" stroke="var(--brand)" fill="var(--brand)" fillOpacity={0.28} isAnimationActive />
            </RadarChart>
          </ResponsiveContainer>
        </div>
        <div className="mt-1">
      ...
      <GlassCard className="p-4 grid gap-3">
        <SectionTitle title="Intelligent soil insights" sub="Observation, reasoning, impact and action for every finding." />
      ...
      {/* Crops */}
      <GlassCard className="p-4 grid gap-3">
        <SectionTitle title="Crop suitability" sub="Ranked against this soil's pH, nutrients, texture, salinity and season." />
        <div className="grid gap-2.5">
          {crops.slice(0, 6).map((c) => (
            <details key={c.crop} className="rounded-xl border border-border p-3 bg-background/40 group">
              <summary className="flex cursor-pointer list-none items-center gap-3">
                <Sprout className={cn("size-4 shrink-0", c.suitability >= 70 ? "text-good" : c.suitability >= 50 ? "text-warn" : "text-bad")} />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-sm font-medium">{c.crop}</span>
                    <span className="font-mono text-sm font-semibold tabular-nums">{c.suitability}%</span>
                  </div>
                  <div className="mt-1.5 h-1.5 rounded-full bg-muted overflow-hidden">
                    <div
                      className={cn("h-full rounded-full", c.suitability >= 70 ? "bg-good" : c.suitability >= 50 ? "bg-warn" : "bg-bad")}
                      style={{ width: `${c.suitability}%`, transition: "width 600ms cubic-bezier(0.22,1,0.36,1)" }}
                    />
                  </div>
                </div>
              </summary>
      ...
      {/* Roadmap */}
      <GlassCard className="p-4 grid gap-3">
        <SectionTitle title="Soil improvement roadmap" sub="Sequenced so each step raises the return on the next." />
      ...
      {trend.length > 1 && (
        <GlassCard className="p-4">
          <SectionTitle title="Historical trend" sub="Soil health score and organic carbon across your saved analyses." />
          <div className="h-56 mt-3 -ml-3">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={trend}>
                <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
                <XAxis dataKey="date" tick={{ fontSize: 10, fill: "var(--muted-foreground)" }} />
                <YAxis yAxisId="l" domain={[0, 100]} tick={{ fontSize: 10, fill: "var(--muted-foreground)" }} width={28} />
                <YAxis yAxisId="r" orientation="right" domain={[0, 2]} tick={{ fontSize: 10, fill: "var(--muted-foreground)" }} width={28} />
                <Tooltip
                  contentStyle={{
                ...
                />
                <Line yAxisId="l" type="monotone" dataKey="score" name="Health score" stroke="var(--brand)" strokeWidth={2.5} dot={{ r: 3 }} />
                <Line yAxisId="r" type="monotone" dataKey="oc" name="Organic carbon %" stroke="var(--accent)" strokeWidth={2} dot={{ r: 3 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </GlassCard>
      ...
      <GlassCard className="p-4 grid gap-2 border-dashed">
        ...
        <ul className="text-xs text-muted-foreground grid gap-1 list-disc pl-5">
          ...
          <li>Scores are modelled from agronomic thresholds, not a guarantee of yield. Field response also depends on weather, variety and management.</li>
        </ul>
      </GlassCard>
...
function FertGroup({ title, items }: { title: string; items: SoilAnalysis["fertilizer"]["organic"] }) {
...
function buildAlerts(record: SoilRecord, score: number, delta: number | null, daysSince: number) {
  ...
  if (m.nitrogen < IDEAL.nitrogen[0] || m.phosphorus < IDEAL.phosphorus[0] || m.potassium < IDEAL.potassium[0]) {
    alerts.push({
      id: "nutrients",
      title: "Nutrients below the recommended level",
      detail: "At least one macronutrient sits under the ICAR low threshold. Apply the corrective dose in the fertiliser plan before sowing.",
    });
  }
  ...
  if (score < 50) {
    alerts.push({
      ...
      detail: "Prioritise organic matter and pH correction — fertiliser applied on this soil will be used inefficiently.",
    });
  }
  return alerts;
}

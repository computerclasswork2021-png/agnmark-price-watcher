...
export const Route = createFileRoute("/what-if")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Model storage, transport and price to see profit and risk impact.",
      },
    ...
    <AppShell>
      <div className="flex flex-col gap-6">
        ...
        <div className="bg-surface ring-1 ring-black/5 rounded-2xl p-5 grid gap-5">
          ...
          />
          <Slider
            label={t("volume_qtl", lang)}
            min={1}
            max={500}
            value={quintals}
            onChange={setQuintals}
          />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <Metric
            ...
            value={formatINR(net, lang)}
            tone={net >= 0 ? "good" : "bad"}
          />
          <Metric
            label={t("added_risk", lang)}
            value={`+${riskDelta}`}
            tone={riskDelta > 20 ? "bad" : riskDelta > 0 ? "warn" : "good"}
          />
          <Metric
            label={t("transport_cost_metric", lang)}
            value={formatINR(transport, lang)}
            tone="muted"
          />
          <Metric
            label={t("storage_cost_metric", lang)}
            value={formatINR(storageCost, lang)}
            tone="muted"
          />
        </div>
      </div>

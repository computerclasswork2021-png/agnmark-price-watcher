...
export const Route = createFileRoute("/analyze")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Multi-day profit chart, disease risk, best sell window and crop-quality breakdown." },
...
function AnalyzePage() {
  ...
  if (hydrated && !profile) {
    return <AppShell><DataUnavailable title={t("begin", lang)} reason={t("action_hero_missing_profile", lang)} /></AppShell>;
  }
    ...
    <AppShell>
      <div className="flex flex-col gap-4">
        ...
        <button onClick={() => mutation.mutate()} disabled={mutation.isPending}
          ...
          {mutation.isPending ? <><Loader2 className="size-4 animate-spin" /> {t("loading", lang)}</> : <><Sparkles className="size-4" /> {mutation.data ? t("regenerate", lang) : t("run_analysis", lang)}</>}
        </button>
...
function AnalysisView({ data, lang }: { data: DeepAnalysis; lang: "en" | "hi" }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <p className="text-sm text-foreground/80 bg-surface ring-1 ring-black/5 rounded-2xl p-4">{data.summary}</p>

      <div className="grid grid-cols-2 gap-3">
        <Stat label={t("yield_forecast", lang)} value={`${data.yieldForecastQuintals} qtl · ${data.yieldForecastQuintals * 100} kg`} reason={data.yieldReasoning} />
        <Stat label={t("disease_risk", lang)} value={`${Math.round(data.diseaseRiskScore)}/100`} reason={data.diseaseRiskReasoning} />
      </div>
      ...
      <div className="bg-surface ring-1 ring-black/5 rounded-2xl p-4">
        ...
        <div className="text-lg font-semibold">
          {lang === "hi" ? `${data.bestSellWindow.startDaysFromNow}–${data.bestSellWindow.endDaysFromNow} दिन में` : `Day ${data.bestSellWindow.startDaysFromNow}–${data.bestSellWindow.endDaysFromNow}`}
        </div>
        <p className="text-xs text-muted-foreground mt-1">{data.bestSellWindow.reasoning}</p>
      </div>

      <div className="bg-surface ring-1 ring-black/5 rounded-2xl p-4">
        <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-3">{t("chart_profit_by_day", lang)}</div>
          ...
          {data.profitByHarvestDay.map((d) => {
            ...
            return (
              <div key={d.dayOffset} className="flex-1 flex flex-col items-center gap-1">
                <div className="text-[9px] font-mono text-muted-foreground">{formatINR(d.expectedProfitRupees, lang)}</div>
                <div className={`w-full rounded-t ${positive ? "bg-brand" : "bg-bad"}`} style={{ height: `${h}%` }} />
      ...
      <div className="bg-surface ring-1 ring-black/5 rounded-2xl p-4 flex flex-col gap-3">
        <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">{t("crop_quality", lang)}</div>

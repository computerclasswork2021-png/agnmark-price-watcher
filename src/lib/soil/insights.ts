...
export function generateInsights(m: SoilMeasurements): Insight[] {
  ...
  if (m.ph < 5.5) {
    push({
      ...
      impact: "Root growth is physically restricted and applied phosphorus largely fails to reach the crop.",
      action: "Apply agricultural lime at 3–4 quintal/acre, incorporate 3–4 weeks before sowing, then re-test pH.",
    });
    ...
    push({
      ...
      action: "Apply lime at 2 quintal/acre or 1 t/acre compost, which buffers pH upward gradually.",
    });
  } else if (m.ph > 8.2) {
    push({
      id: "ph-alkaline",
      ...
      impact: "Interveinal yellowing (zinc/iron chlorosis) and reduced grain filling, especially in rice and wheat.",
      action: "Apply gypsum at 2 quintal/acre, use ammonium sulphate instead of urea, and add 25 kg/acre zinc sulphate.",
    });
  } else {
    push({
      id: "ph-ok",
      ...
      reasoning: "Between pH 6.0 and 7.5 all major and most micronutrients are simultaneously plant-available.",
  ...
  if (m.nitrogen < IDEAL.nitrogen[0]) {
    push({
      ...
      action: "Split urea across basal, tillering and panicle stages, and add FYM to rebuild the mineralisable pool.",
    });
    ...
    push({
      ...
      reasoning: "Surplus nitrate is mobile, leaches below the root zone and pushes vegetative growth over reproductive growth.",
      impact: "Lodging, delayed maturity, higher sucking-pest pressure and wasted input money.",
      action: "Skip the basal nitrogen dose this season and monitor crop colour before any top dressing.",
    });
  }
  ...
  if (m.phosphorus < IDEAL.phosphorus[0]) {
    push({
      ...
      reasoning: "Phosphorus drives ATP transfer and early root proliferation; it is immobile, so the crop can only take up what sits near the root.",
      impact: "Weak root systems, purple leaf tinge and poor grain set.",
      action: "Place DAP or SSP in the root zone as a full basal dose and inoculate with phosphate-solubilising bacteria.",
    });
  }
  if (m.potassium >= IDEAL.potassium[0] && m.potassium <= IDEAL.potassium[1]) {
    push({
      ...
      reasoning: "Potassium regulates stomatal opening and phloem transport; at this level the crop retains its drought and disease buffer.",
    ...
    push({
      ...
      reasoning: "Below 120 kg/ha the crop cannot maintain osmotic regulation under midday water stress.",
  ...
  if (m.organicCarbon < IDEAL.organicCarbon[0]) {
    push({
      ...
      impact: "Surface crusting, faster drying, and a permanent dependence on purchased fertiliser.",
      action: "Apply 2–3 t/acre FYM or 1 t/acre vermicompost and grow a green-manure legume before the main crop.",
    });
  }
  ...
  if (m.waterHoldingCapacity >= IDEAL.waterHoldingCapacity[0]) {
    ...
    push({
      ...
      reasoning: "Coarse texture and low organic matter leave few micro-pores to hold water against gravity.",
      impact: "Frequent short irrigations required; nutrients leach past the root zone with each event.",
      action: "Add organic matter, mulch heavily and shift to drip or shorter, more frequent irrigation.",
    });
  }
  ...
  if (m.ec > 1) {
    push({
      ...
      reasoning: "Dissolved salts lower the osmotic potential of the soil solution, so roots must work against the soil to extract water.",
      impact: m.ec > 4 ? "Germination failure and 25–50% yield loss in sensitive crops." : "Mild osmotic stress; pulses and vegetables will underperform.",
      action: "Leach with good-quality water, apply gypsum if sodium-dominated, and grow salt-tolerant crops meanwhile.",
    });
  }
  ...
  if (ratio > 3 || ratio < 0.7) {
    push({
      ...
      impact: "Nutrient antagonism at the root surface reduces uptake of whichever element is in relative shortage.",
      action: "Correct the deficient element first, then re-balance the remaining dose rather than raising both.",
    });
  }

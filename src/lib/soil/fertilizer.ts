 ...
 */
export function fertilizerPlan(m: SoilMeasurements, acres: number, tier: Tier = "middle"): FertilizerPlan {
  ...
  const perAcre = (kgPerHa: number, concentration: number) => round25((kgPerHa / 2.47 / concentration) * area);
  ...
  if (pGap > 0) {
    ...
    chemical.push({
      ...
      reasoning: "Phosphorus is immobile in soil, so band placement near the seed outperforms broadcasting by a wide margin.",
    });
  }
  if (kGap > 0) {
  ...
  if (m.organicCarbon < IDEAL.organicCarbon[1]) {
    ...
    organic.push({
      ...
      benefit: "Raises organic carbon roughly 0.05–0.1% per season and improves aggregate stability.",
    ...
    organic.push({
      ...
      reasoning: "Vermicompost mineralises faster than raw FYM, giving a visible response inside the same season.",
    });
  }
  ...
  bio.push({
    ...
    reasoning: "Nitrogen-fixing inoculants perform best where organic carbon supports a living microbial population.",
  });
  bio.push({
    ...
    reasoning: "Most Indian soils hold large fixed-P reserves; PSB converts a fraction of it to plant-available form.",
  });

  if (m.nitrogen > IDEAL.nitrogen[1]) {
    warnings.push("Nitrogen already exceeds the agronomic optimum. Applying more urea will cause lodging and nitrate leaching without any yield gain.");
  }
  if (m.phosphorus > IDEAL.phosphorus[1]) {
    warnings.push("Phosphorus is above the response threshold. Extra DAP will induce zinc deficiency instead of raising yield.");
  }
  if (m.ec > 2) {
    warnings.push("Soil EC is elevated. Heavy chemical fertiliser will raise salinity further — leach first, then fertilise.");
  }
  if (tier === "low" && chemical.length > 2) {
    warnings.push("On a limited budget, prioritise the nitrogen dose and organic matter first; potash can wait one season.");
  }
...
export function improvementRoadmap(m: SoilMeasurements): RoadmapStep[] {
  ...
  if (m.organicCarbon < IDEAL.organicCarbon[0]) {
    steps.push({
      ...
      detail: "Spread 2–3 t/acre of well-rotted FYM or 1 t/acre vermicompost and incorporate to 15 cm.",
  ...
  if (m.moisture < IDEAL.moisture[0] || m.waterHoldingCapacity < IDEAL.waterHoldingCapacity[0]) {
    steps.push({
      ...
      detail: "Mulch with crop residue at 2 t/acre and shift to shorter, more frequent irrigations or drip.",
  ...
  steps.push({
    ...
    detail: "Sow dhaincha or sunhemp on fallow land, or treat seed with Rhizobium/Azospirillum before sowing.",
  ...
  steps.push({
    week: week + 1,
    title: "Re-test soil",
    detail: "Collect 8–10 cores in a zig-zag pattern from 0–15 cm, mix, quarter to 500 g and submit to the nearest soil testing lab.",
